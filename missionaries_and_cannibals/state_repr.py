...
class MC_state:
    ...
    def goal_test(self) -> bool:
        return self.islegal and self.left_m == MAX_NUM and self.left_c == MAX_NUM
    ...
    def successors(self) -> List[MC_state]:
        ...
        if self.boat:
            if self.left_m > 0:
                result.append(MC_state(self.left_m - 1, self.left_c, not boat))
            if self.left_c > 0:
                result.append(MC_state(self.left_m, self.left_c - 1, not boat))
            if self.left_c > 0 and self.left_m > 0:
                result.append(MC_state(self.left_m - 1, self.left_c - 1, not boat))    
        else:
            if self.right_m > 0:
                result.append(MC_state(self.left_m + 1, self.left_c, not boat))
            if self.right_c > 0:
